...
class Spider(object):
    ...
    def html2res(self, url, date, i=1):
        ...
        title = soup.find('div', {'class': 'article-header'}).find('h1').get_text().strip()
        ...
        html = soup.find('div', {'class': 'article-body'})
        # print(article)
    ...
    def run(self):        
        ...
        try:
            ...
            while i < self.PAGE_NUM:
                ...
                url = 'https://news.cnr.cn/native/gd/sz/index_{}.html'.format(i)
                resp = requests.get(url, headers = {'headers': self.ua.random})  

                #获取近一天内文章网址
                soup = BeautifulSoup(resp.content, 'lxml')
                newsList = soup.find('div', {'class': 'articleList'}).find_all('a')
                ...
                for new in newsList:
                    ...
                    if flag:
                        break
                    # 获取符合时间要求的链接地址 ：
                    content_url = new['href']
                    print('链接:\t', content_url)
                    time.sleep(random.randint(2, 3))

                    ori_new_time = new.find('span', {'class': 'publishTime'}).get_text().strip()
                    print(ori_new_time)

                    
                    date = datetime.datetime.strptime(str(ori_new_time) + ":00", "%Y-%m-%d %H:%M:%S")
                    # self.args.start <= 
                    if date <= self.args.end:  #时间在爬取范围内，进行内容提取
