...
class Spider(object):
    ...
    def html2res(self, url, date, i=1):
        ...
        title = soup.find('h1', {'class': 'mb-90'}).get_text().strip()
        ...
        html = soup.find('div', {'class': 'story-container'})
        # print(article)
    ...
    def run(self):        
        ...
        try:
            ...
            while i < self.PAGE_NUM:
                ...
                url = 'https://digitalpower.huawei.com/cn/all/news/all_category/{}.html'.format(i)
                resp = requests.get(url, headers = {'headers': self.ua.random})  

                #获取近一天内文章网址
                soup = BeautifulSoup(resp.content, 'lxml')
                newsList = soup.find('div', {'class': 'row mb50 news-list-box'}).find_all('div', {'class': 'col-md-4'})
                ...
                for news in newsList:
                    ...
                    if flag:
                        break
                    # 获取符合时间要求的链接地址 ：
                    tempUrl = news.find('a')['href']
                    officialUrl = 'https://digitalpower.huawei.com'
                    content_url = officialUrl + tempUrl
                    print('链接:\t', content_url)

                    time.sleep(random.randint(2, 3))

                    ori_new_time = news.find('p').get_text().strip().replace(' ', '').replace('年', '-').replace('月', '-').replace('日', '')
                    print(ori_new_time)
