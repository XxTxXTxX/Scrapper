...
class Spider(object):
    ...
    def html2res(self, url, date, i=1):
        ...
        title = soup.find('div', {'class': 'art-con'}).find('h1').get_text().strip()
        ...
        html = soup.find('div', {'class': 'art-txt'})
        # print(article)
    ...
    def run(self):        
        ...
        try:
            ...
            while i < self.PAGE_NUM:
                ...
                url = 'http://www.hedianzhan.com.cn/tag/%E4%B8%AD%E5%9B%BD%E5%85%85%E7%94%B5%E8%81%94%E7%9B%9F'
                resp = requests.get(url, headers = headers)  

                #获取近一天内文章网址
                soup = BeautifulSoup(resp.content, 'lxml')
                newsList = soup.find('div', {'class': 'article_box'}).find_all('div', {'class': 'items'})
                ...
                for news in newsList:
                    ...
                    if flag:
                        break
                    # 获取符合时间要求的链接地址 ：
                    tempUrl = news.find('div', {'class': 'content-box'}).find('div', {'class': 'posts-gallery-content'}).find('h2').find('a')['href']
                    officialUrl = 'http://www.hedianzhan.com.cn/'
                    content_url = officialUrl + tempUrl
                    print('链接:\t', content_url)
                    time.sleep(random.randint(2, 3))
                    newC = requests.get(content_url, headers = headers)
                    newSoup = BeautifulSoup(newC.content, 'lxml')

                    ori_new_time =newSoup.find('li', {'class': 'ico-time'}).get_text().strip().replace(' ', '').replace('年', '-').replace('月', '-').replace('日', '')
                    print(ori_new_time)
