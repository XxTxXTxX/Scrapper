...
class Spider(object):
    ...
    def html2res(self, url, date, i=1):
        ...
        title = soup.find('h1', {'class': 'pagetitle'}).get_text().strip()
        ...
        html = soup.find('div', {'class': 'newscontent'})
        # print(article)
    ...
    def run(self):        
        ...
        try:
            ...
            while i < self.PAGE_NUM:
                ...
                url = 'http://www.chinanev.net/news?page={}'.format(i)
                resp = requests.get(url, headers = {'headers': self.ua.random})  

                #获取近一天内文章网址
                soup = BeautifulSoup(resp.content, 'lxml')
                newsList = soup.find('div', {'class': 'news_content'}).find_all('a')
                ...
                for new in newsList:
                    ...
                    tempUrl = new['href']
                    print('链接:\t', tempUrl)
                    officialUrl = 'http://www.chinanev.net/'
                    ...
                    ori_new_time = new_soup.find('div', {'class': 'page_time'}).get_text().strip()
                    print(ori_new_time)
